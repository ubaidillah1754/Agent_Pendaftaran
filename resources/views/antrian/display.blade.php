    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        body {
            ...
            color:var(--cream);min-height:100vh;margin:0;display:flex;flex-direction:column;
        ...
        /* ===== Header ===== */
        .header {
            padding:20px 44px;border-bottom:1px solid rgba(232,199,102,.2);
        ...
        .header .logo-badge {
            ...
            border:2px solid var(--gold-lite); flex-shrink:0;
            box-shadow: 0 0 0 5px rgba(201,162,39,.08);
        }
        .header .logo-badge img { width:100%;height:100%;object-fit:contain; }
        .header h1 {
            margin:0;font-family:'Amiri',serif;font-size:1.5rem;font-weight:700;
            letter-spacing:.01em;
        }
        .header .poli {
            font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.14em;
            color:var(--gold-lite);margin-top:2px;
        }
        .clock-wrap { text-align:right; }
        .clock {
            font-family:'Spectral',serif; font-variant-numeric: tabular-nums;
            font-size:2.1rem;font-weight:700;color:var(--gold-lite);line-height:1;
        }
        .date-str { font-size:.78rem;color:rgba(251,248,240,.5);margin-top:4px; }

        /* ===== Main split ===== */
        .main { flex:1;display:grid;grid-template-columns:1.1fr 1fr;gap:0; }
        .panel-left {
            padding:36px 44px 44px;border-right:1px solid rgba(232,199,102,.16);
            display:flex; flex-direction:column; align-items:center; justify-content:center;
            text-align:center; gap:6px;
        }
        .panel-right { padding:36px 44px 44px; display:flex; flex-direction:column; }

        .eyebrow {
            font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.18em;
            color:rgba(251,248,240,.5); display:flex; align-items:center; gap:8px;
            margin-bottom:22px;
        }
        .eyebrow i { color:var(--gold-lite); }

        /* ===== Signature element: mihrab arch call panel ===== */
        .arch-stage { position:relative; width:340px; height:340px; display:flex; align-items:center; justify-content:center; }
        .arch-ring {
            position:absolute; border-radius:50%; border:1px solid rgba(232,199,102,.22);
            animation: pulse-ring 2.8s ease-out infinite;
        }
        .arch-ring.r1 { inset:0; }
        .arch-ring.r2 { inset:0; animation-delay: .9s; }
        .arch-ring.r3 { inset:0; animation-delay: 1.8s; }
        ...
        .arch-frame {
            position:relative; z-index:2; width:250px; height:280px;
        ...
        .arch-frame::before {
            content:""; position:absolute; inset:10px; border:1px solid rgba(232,199,102,.35);
            border-radius: 115px 115px 8px 8px; pointer-events:none;
        }
        .current-number {
            font-family:'Spectral',serif; font-size:5.6rem;font-weight:800;line-height:1;
            color:var(--gold-lite); text-shadow:0 0 40px rgba(201,162,39,.5);
        }
        .current-tag { font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.16em;color:rgba(251,248,240,.55);margin-top:10px; }
        .current-name { font-size:1.55rem;font-weight:700;margin-top:20px;font-family:'Amiri',serif; }
        .current-meta {
            font-size:.85rem;color:var(--gold-lite);margin-top:8px;
            display:flex; align-items:center; gap:6px;
        }

        .no-antrian { color:rgba(251,248,240,.4); }
        .no-antrian i { font-size:3.6rem;display:block;margin-bottom:14px; color:rgba(232,199,102,.4); }

        /* ===== Next-up ticket list ===== */
        .next-list { display:flex; flex-direction:column; gap:12px; overflow:hidden; }
        .next-item {
            background:linear-gradient(90deg, rgba(255,255,255,.055), rgba(255,255,255,.02));
            border:1px solid rgba(232,199,102,.14); border-left:3px solid var(--gold);
            border-radius:12px;padding:14px 20px;
            display:flex;align-items:center;gap:18px;
        }
        .next-no {
            font-family:'Spectral',serif;
            font-size:1.5rem;font-weight:700;color:var(--gold-lite);min-width:56px;
            text-align:center;
        }
        .next-divider { width:1px; height:26px; background:rgba(232,199,102,.2); }
        .next-name { font-size:1rem;font-weight:600; }
        .next-item:first-child { border-left-color: var(--emerald-lite); box-shadow: 0 0 22px rgba(18,148,110,.12); }

        .empty-state { text-align:center;color:rgba(251,248,240,.35);margin-top:50px; }
        .empty-state i { font-size:3rem;display:block;margin-bottom:10px; }

        /* ===== Footer ===== */
        .footer {
            padding:14px 44px;border-top:1px solid rgba(232,199,102,.16);
            display:flex;justify-content:space-between;align-items:center;
            font-size:.76rem;color:rgba(251,248,240,.4);
        }
        .footer .refresh-dot {
            display:inline-block;width:7px;height:7px;border-radius:50%;
            background:var(--gold-lite);margin-right:7px;
            animation: blink-dot 1.6s ease-in-out infinite;
        }
        @keyframes blink-dot { 0%,100%{opacity:1}50%{opacity:.25} }

        @media (prefers-reduced-motion: reduce) {
            .arch-ring, .footer .refresh-dot { animation: none; }
        }
    </style>
</head>
<body>
...
<div class="footer">
    ...
    <span><span class="refresh-dot"></span>Halaman ini refresh otomatis setiap 15 detik</span>
</div>
